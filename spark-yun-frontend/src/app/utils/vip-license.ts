...
export function filterVipMenus(menuList: Menu[], vipEnabled: boolean, licenseApiAvailable = true): Menu[] {
    ...
    menuList.forEach((menu) => {
        ...
        const isVipMenu = isVipMenuCode(menu.code)
        if (isVipMenu && !vipEnabled) {
            return
        }

        const hasChildren = !!menu.children?.length
        const children = hasChildren ? filterVipMenus(menu.children || [], vipEnabled, licenseApiAvailable) : undefined
