...
import { getVipLicenseEnabled, resetVipLicenseCache } from '@/app/utils/vip-license'
...
export function useMenuAvatar() {
    ...
    return {
        renderMenuAvatar: () => (
            <>
                <el-popover
                    v-model:visible={menuVisible.value}
                    placement="top-end"
                    show-arrow={false}
                    trigger="click"
                    popper-class="zyq-home__menu-avatar"
                >
                    {{
                        reference: () => (
                            <el-avatar class="zyq-home__avatar" size={32}>
                                {username.value}
                            </el-avatar>
                        ),
                        default: () => (
                        ...
                        {filteredTenantList.value.map((tenant) => (
                            ...
                            >
                                ...
                                {authStore.tenantId === tenant.id ? (
                                    <span class="zyq-home__tenant-current">当前</span>
                                ) : null}
                            </div>
                        ))}
