...
function workspaceDefaultRoute(): RouteLocationRaw {
  ...
  return target ? { name: target.name } : { name: 'forbidden' }
}
...
function defaultRoute(): RouteLocationRaw {
  ...
  return authStore.userInfo?.systemAdmin ? { name: 'platform' } : { name: 'workspace' }
}
