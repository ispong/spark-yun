      ...
      <template v-if="formData.cronConfig.enable">
        <div class="cron-config">
          <el-form-item label="调度配置">
            <el-radio-group
              v-model="formData.cronConfig.setMode"
              size="small"
          ...
          <el-form-item
            v-if="formData.cronConfig.setMode === 'ADVANCE'"
            label="cron表达式"
            prop="cronConfig.cron"
          >
            <el-input
          ...
          <template v-else>
            <el-form-item
              label="调度周期"
              prop="cronConfig.range"
            >
              <el-select
                v-model="formData.cronConfig.range"
                placeholder="请选择"
            ...
            <template v-if="formData.cronConfig.range === 'min'">
              ...
              <el-form-item
                label="时间间隔（分钟）"
                prop="cronConfig.minNum"
              >
                <el-input-number
                  v-model="formData.cronConfig.minNum"
            ...
            <template v-if="formData.cronConfig.range === 'hour'">
              ...
              <el-form-item
                label="时间间隔（小时）"
                prop="cronConfig.hourNum"
              >
                <el-input-number
                  v-model="formData.cronConfig.hourNum"
                  class="time-num-input"
                  :disabled="!formData.cronConfig.enable"
                  placeholder="请输入"
                  :min="0"
            ...
            <template v-if="formData.cronConfig.range === 'month'">
              ...
              <el-form-item
                label="指定时间"
                prop="cronConfig.monthDay"
              >
                <el-select
                  v-model="formData.cronConfig.monthDay"
                  :disabled="!formData.cronConfig.enable"
                  placeholder="请选择"
                >
                  <el-option
            ...
            <template v-if="formData.cronConfig.range === 'week'">
              ...
              <el-form-item
                label="指定时间"
                prop="cronConfig.weekDate"
              >
                <el-select
                  v-model="formData.cronConfig.weekDate"
                  placeholder="请选择"
                  :disabled="!formData.cronConfig.enable"
                >
                  <el-option
...
function getCron() {
    if (formData.cronConfig.range === 'min') {
        ...
        state.hoursText = `${formData.cronConfig.startDateMin.split(':')[0]}-${
            formData.cronConfig.endDateMin.split(':')[0]
        }`
    } else if (formData.cronConfig.range === 'hour') {
        // 调度周期为小时
        state.secondsText = '0'
        state.minutesText = '0'
        state.hoursText = `${formData.cronConfig.startDate.split(':')[0]}-${
            formData.cronConfig.endDate.split(':')[0]
        }/${formData.cronConfig.hourNum}`
        ...
        GetDatasourceList({
            ...
            .then((res: any) => {
                dataSourceList.value = res.data.content
                    .filter((item: any) => item.dbType !== 'KAFKA' && formData.dbType == item.dbType)
                    .map((item: any) => {
                        return {
                            label: item.name,
                            value: item.id
