...
const hasSourceConnectConfig = computed(() => {
    return Object.keys(formData.sourceTableAdvanceConfig || {
}).length > 0
})

const hasTargetConnectConfig = computed(() => {
    return Object.keys(formData.targetTableAdvanceConfig || {
}).length > 0
})
...
function openAdvancedConfig(type: 'source' | 'target') {
    ...
    connectConfigList.value = Object.entries(currentConfig || {
}).map(([ key, value ]) => ({
        key,
