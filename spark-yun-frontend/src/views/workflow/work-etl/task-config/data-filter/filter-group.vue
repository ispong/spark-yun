  ...
  <div class="filter-group-container">
    ...
    >
      ...
      >
        ...
        >
          {{ items[index].filterWay || 'AND' }}
        </span>
      </div>
      <!-- 分组条件 -->
      <div
        v-if="item.groupFilter"
        class="filter-group-block"
        :style="getGroupStyle()"
      >
        <el-icon
          v-if="items.length > 1"
        ...
        <div class="filter-group-actions">
          <el-button
            ...
            type="primary"
            size="small"
            @click="addConditionToGroup(item.groupFilter)"
          >
            + 条件
          </el-button>
          <el-button
            ...
            type="primary"
            size="small"
            @click="addGroupToGroup(item.groupFilter)"
          >
            + 分组
          </el-button>
        </div>
      ...
      >
        ...
        <template v-if="item.filterType === 'CONDITION_FILTER'">
          <el-form-item
            :prop="`${propPrefix}[${index}].filterColumn`"
            :rules="rules.filterColumn"
          >
            <el-select
              v-model="items[index].filterColumn"
              filterable
              clearable
              placeholder="字段"
            >
              <el-option
                v-for="opt in colNameOptions"
                :key="opt.value"
                :label="opt.label"
                :value="opt.value"
              />
          ...
          <el-form-item
            :prop="`${propPrefix}[${index}].filterCondition`"
            :rules="rules.filterCondition"
          >
            <el-select
              v-model="items[index].filterCondition"
              placeholder="条件"
            >
              <el-option
                v-for="opt in FilterConditionOptions"
                :key="opt.value"
                :label="opt.label"
                :value="opt.value"
              />
          ...
          >
            <el-input
              v-model="items[index].filterValue"
              clearable
              placeholder="值"
            />
          </el-form-item>
        ...
        <template v-else>
          <el-form-item
            :prop="`${propPrefix}[${index}].customFilter`"
            :rules="rules.customFilter"
            class="custom-form-item"
          >
            <el-input
              v-model="items[index].customFilter"
              clearable
              placeholder="例如：age > 12 AND name = 'test'"
            />
          </el-form-item>
        </template>
        <el-icon
