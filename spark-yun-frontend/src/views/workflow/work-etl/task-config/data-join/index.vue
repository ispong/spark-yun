...
function getMainTableFields(e: boolean) {
    ...
    if (e && currentItem.data.outColumnList) {
        mainTableFields.value = (currentItem.data.outColumnList || [])
...
function getConditionTableFields(e: boolean, element: any) {
    ...
    if (e && currentItem && currentItem.data.outColumnList) {
        conditionTableFields.value = (currentItem.data.outColumnList || [])
            .filter((item: any) => item.checked !== false)
            .map((column: any) => {
                return {
                    label: column.colName,
...
function addNewCode() {
    ...
    addCodeRef.value.showModal(
        (params: any) => {
            formData.value.outColumnList.push({
                ...params
            })
            tableConfig.tableData = formData.value.outColumnList
        },
