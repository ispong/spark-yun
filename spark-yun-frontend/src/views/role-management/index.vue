...
import { DeleteRole, GetPermissionCatalog, PageRole, SaveRole } from '@/services/authorization.service'
...
const catalog = reactive({ modules: [] as string[], actions: [] as string[] })
const form = reactive({
  id: '',
  name: '',
  code: '',
...
function saveRole() {
  ...
  saving.value = true
  SaveRole({ ...form, id: form.id || undefined })
    .then((res: any) => {
...
function removeRole(role: any) {
  ElMessageBox.confirm(`确定删除角色“${role.name}”吗？`, '提示', { type: 'warning' }).then(() => {
    DeleteRole({ roleId: role.id }).then((res: any) => {
      ElMessage.success(res.msg)
      loadRoles()
    })
  })
