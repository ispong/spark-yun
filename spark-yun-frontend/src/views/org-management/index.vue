  ...
  <div class="zqy-seach-table">
    ...
    <el-table v-loading="loading" :data="orgs" row-key="id">
      ...
      <el-table-column label="父组织" min-width="160">
        <template #default="{ row }">{{ orgName(row.parentId) || '-' }}</template>
      </el-table-column>
      <el-table-column label="成员数" width="100">
        <template #default="{ row }">{{ row.userIds?.length || 0 }}</template>
      </el-table-column>
      <el-table-column label="角色数" width="100">
        <template #default="{ row }">{{ row.roleIds?.length || 0 }}</template>
      </el-table-column>
...
function saveOrg() {
  ...
  saving.value = true
  SaveOrg({ ...form, id: form.id || undefined, parentId: form.parentId || undefined })
    .then((res: any) => {
...
function removeOrg(org: any) {
  ElMessageBox.confirm(`删除“${org.name}”后将解除成员和角色关系，是否继续？`, '提示', {
    ...
    DeleteOrg({ orgId: org.id }).then((res: any) => {
      ElMessage.success(res.msg)
      loadData()
    })
  })
