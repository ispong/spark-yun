...
分页模版：
{
  ...
  "count": "${count.long}"
    ...
    >
      ...
      >
        <span class="add-btn">
          <el-icon @click="addNewOption(formDataTest.headerConfig)"
            ><CirclePlus
          /></el-icon>
        </span>
        <div class="form-options__list">
            ...
            <div class="option-btn">
              <el-icon
                v-if="
                  ...
                  formDataTest.headerConfig.length > 1
                "
                class="remove"
      ...
      >
        <span class="add-btn">
          <el-icon @click="addNewOption(formDataTest.bodyConfig)"
            ><CirclePlus
          /></el-icon>
        </span>
        <div class="form-options__list">
          ...
          >
            ...
            <div class="option-btn">
              <el-icon
                v-if="
                  formDataTest.bodyConfig && formDataTest.bodyConfig.length > 1
                "
                class="remove"
...
import { GetCustomApiDetailData, TestCustomApiData } from "../../api";
    ...
    .then((res: any) => {
      if (type === "test") {
        ...
        if (res.data.apiType === "POST") {
          formDataTest.bodyParams = jsonFormatter(
            JSON.parse(res.data.reqJsonTemp),
          );
        } else {
      ...
      accessRuleList.value = res.data.content.map((item: any) => {
        return {
          label:
            item.name +
            "（" +
            (item.ruleType === "WHITELIST" ? "白名单" : "黑名单") +
            "）",
          value: item.id,
        };
      });
    ...
    TestCustomApiData({
      ...
      .then((res: any) => {
        ...
        if (res.data.httpStatus === 200) {
          formDataTest.returnConfig = jsonFormatter(
