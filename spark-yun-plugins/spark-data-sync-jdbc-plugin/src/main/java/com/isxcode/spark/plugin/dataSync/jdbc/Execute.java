...
public class Execute {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (SparkSession sparkSession = initSparkSession(pluginReq.getSparkConfig())) {
            ...
            // 执行sql同步语句
            if (DatasourceType.HIVE.equals(pluginReq.getSyncWorkConfig().getSourceDBType())
                && Strings.isNotEmpty(pluginReq.getSyncWorkConfig().getQueryCondition())) {
    ...
    private static void registerJavaUdaf(SparkSession sparkSession, String funcName, String className) {
        ...
        try {
            ...
            sparkSession.udf().register(funcName, (org.apache.spark.sql.expressions.UserDefinedAggregateFunction) udaf);
        } catch (Exception e) {
