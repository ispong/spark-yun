...
if ! command -v java &>/dev/null; then
  nohup "$JAVA_HOME"/bin/java -Xms1g -Xmx2g -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath="${agent_path}"/logs/ -XX:+UseG1GC -XX:MaxMetaspaceSize=256m -jar "${agent_path}"/lib/zhiqingyun-agent.jar --server.port="${agent_port}" --spring.config.additional-location="${agent_path}"/conf/ > /dev/null 2>&1 &
else
  nohup java -Xms1g -Xmx2g -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath="${agent_path}"/logs/ -XX:+UseG1GC -XX:MaxMetaspaceSize=256m -jar "${agent_path}"/lib/zhiqingyun-agent.jar --server.port="${agent_port}" --spring.config.additional-location="${agent_path}"/conf/ > /dev/null 2>&1 &
fi
