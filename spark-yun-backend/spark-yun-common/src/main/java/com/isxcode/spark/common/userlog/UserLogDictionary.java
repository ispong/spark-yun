...
public class UserLogDictionary {
    ...
    private static final List<UserLogDefinition> DEFINITIONS = List.of(
        definition(UserLogType.PLATFORM, "USER", "用户中心", "ADD", "新增", "新增用户"),
    ...
    public static List<UserLogDefinition> modules() {
        ...
        return DEFINITIONS.stream().collect(Collectors.toMap(UserLogDefinition::getModuleCode, Function.identity(),
            (left, right) -> left)).values().stream().toList();
    }

    private static UserLogDefinition definition(String logType, String moduleCode, String moduleName,
        String actionCode, String actionName, String apiName) {
